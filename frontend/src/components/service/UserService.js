...
class UserService {
  ...
  static async getUserById(identification, token) {
    try {
      ...
      console.error(
        `Error al obtener usuario con identificación ${identification}:`,
        error.response?.data || error.message
      );
      throw error.response?.data || error.message;
    }
  ...
  static async deleteUser(identification, token) {
    try {
      const response = await this.api.delete(
        `/admin/delete-user/${identification}`,
        {
          headers: this.getAuthHeaders(token),
        }
      );
      ...
      console.error(
        `Error al eliminar usuario con identificación ${identification}:`,
        error.response?.data || error.message
      );
      throw error.response?.data || error.message;
    }
  ...
  static async updatePassword(identification, passwordData, token) {
    try {
      ...
      console.error(
        `Error al actualizar la contraseña del usuario ${identification}:`,
        error.response?.data || error.message
      );
      throw error.response?.data || error.message;
    }
